...
DOCX uses explicit 18-point paragraph indentation and 18-mm margins from the flow recipe, repeated table headers, and a static chart and pivot result. Native PDF uses its declared Helvetica flow profile and the same required content. It does not claim Word pagination equivalence. Static chart and PDF text support the declared Windows Latin font profile; unsupported glyphs block visibly. PPTX keeps the core fixture on two slides with an editable native chart and tables. Required content can continue across additional table slides rather than disappear. Unsupported chart types and pivot configurations block.

Bound report images use normalized, metadata-free PNG bytes. Before creating any output, the exporter checks the supplied resolver returns bytes whose SHA-256 and decoded dimensions exactly match each node's frozen render identity. The original asset's digest remains separately bound in the manifest. No image URL or source path is opened. Missing legacy render identities, unavailable or altered blobs, invalid dimensions, animation and unnormalized metadata block explicitly. Limits are four image nodes per view, 20 MiB per PNG, 8192 pixels per side and 16 million pixels.

Office exports embed the original normalized PNG bytes, preserve aspect ratio without cropping or upscaling, and retain DrawingML alt text, title and decorative metadata. Image placement is editable; raster pixel content is not. Images appear as final flow sections, on a dedicated Images worksheet, and on additional presentation slides. PDF embeds image pixels with visible nondecorative captions and records alt text in the manifest; it explicitly does not certify tagged-PDF image accessibility.
...
Run `.venv/bin/python -m pytest tests/test_exports.py tests/test_image_exports.py -q`. Tests inspect real files through independent readers and exercise exact values, coverage, native chart/pivot parts, approved embedded datasets, formula-like text, strict capability gates, contradictory pivot caches, immutable output bytes, and preview failure handling. Image tests inspect embedded bytes and pixels, actual Office geometry, alt/decorative metadata, resolver integrity and fail-closed behavior. Visual QA also renders all four fixture formats. The acceptance unit is the resulting file and its declared capability scope, not merely a valid ZIP or successful function call.
